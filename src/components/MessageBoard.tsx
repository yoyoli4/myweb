...
export default function MessageBoard() {
  ...
  // 静态托管（无服务端 /api）时，提问接口不可用，优雅降级不崩页
  ...
  const submit = async () => {
    ...
    if (apiOffline) {
      alert('当前为静态页面，提问功能需要服务端支持，暂不可用。')
      return
    }
    setSubmitting(true)
    try {
  ...
  const fmt = (t: number) =>
    new Date(t).toLocaleString('zh-CN', { month: '2-digit', day: '2-digit' })
  ...
  return (
    <div className="mx-auto max-w-3xl px-4">
      ...
      <div className="flex flex-col items-center">
        ...
        <span className="mt-2 text-xs tracking-[0.3em] text-mute">提 问</span>
      </div>
            ...
            {sent ? (
              <div className="py-6 text-center">
                <p className="text-sm leading-relaxed text-paper/90">
                  问题已丢进箱子。
                  <br />
                  <span className="text-mute">只有我能看到，回答之后才会展示在这里。</span>
                </p>
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="mt-5 rounded-full border border-gold/50 px-5 py-1.5 text-xs tracking-[0.2em] text-gold-bright hover:bg-gold/10"
                >
                  好
                </button>
              </div>
      ...
      <div className="mt-10 space-y-4">
        {loading && <p className="text-center text-sm text-mute">正在翻箱子……</p>}
        {!loading && questions.length === 0 && (
          <p className="text-center text-sm text-mute">
            {apiOffline ? '提问箱需要服务端支持，当前静态页面暂不可用。' : '还没有回答过的问题，先丢一个进来吧。'}
          </p>
        )}
        {questions.map((q) => (
          <div key={q.id} className="rounded-xl border border-gold/15 bg-ink/40 p-4 sm:p-5">
            ...
            <div className="flex items-baseline justify-between gap-2">
              <span className="text-[11px] tracking-[0.2em] text-mute/70">{q.nickname} 问</span>
              <span className="text-[11px] text-mute/50">{fmt(q.createdAt)}</span>
            </div>
            {q.text && <p className="mt-2 whitespace-pre-wrap text-sm leading-relaxed text-paper/90">{q.text}</p>}
            {q.audio && (
              <audio src={q.audio} controls className="mt-3 h-8 w-full max-w-md" />
            )}

            {/* 我的回答 */}
            {q.answer && (
              <div className="mt-3 rounded-lg border border-gold/10 bg-gold/[0.04] p-3">
                <span className="text-[11px] tracking-[0.2em] text-gold/80">我 答</span>
